...
export class StatusPluginCtrl extends MetricsPanelCtrl {
	...
	handleThresholdStatus(series, target) {
		...
		if (isCheckRanges) {
			...
			else if (series.display_value == series.threshold.on) {
			    isOn = true;
			}
		}
	...
	handleCssDisplay() {
		...
		} else if (this.on.length > 0) {
		    this.$panelContainer.addClass('on-state');
	...
	handleMaxAlertsToShow() {
		if(this.panel.maxAlertNumber != null && this.panel.maxAlertNumber >= 0) {
			...
			let arrayNamesToSlice = ["disabled", "crit", "warn", "on", "display"];
	...
	static parseThresholds(metricOptions) {
		...
		res.onIsNumber = StatusPluginCtrl.isFloat(metricOptions.on)
		if(res.onIsNumber) {
		    res.on = parseFloat(metricOptions.on);
		} else {
		    res.on = metricOptions.on;
		}
	...
	autoFlip() {
		if (this.timeoutId) clearInterval(this.timeoutId);
		if (this.panel.flipCard && (this.crit.length > 0 || this.warn.length > 0 || this.on.length > 0 || this.disabled.length > 0)) {
